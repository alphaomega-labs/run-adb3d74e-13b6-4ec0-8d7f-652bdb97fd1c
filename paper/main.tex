...
\end{equation}
where $G=(V,E)$ is a finite directed acyclic phase graph, $\mathcal{C}=\{C_e\}_{e\in E}$ is an edge-contract family, $\Phi$ is a validator family, $\Pi$ is a meta-orchestration policy, $\mathcal{M}$ is persistent memory, and $\mathcal{T}$ is an append-only execution trace. For an edge $e=(u,v)$, we use
\begin{equation}
C_e=(\tau^{\mathrm{in}}_e,\tau^{\mathrm{out}}_e,\mathrm{pre}_e,\mathrm{post}_e),
\label{eq:edge_contract}
\end{equation}
where type signatures $\tau^{\mathrm{in}}_e,\tau^{\mathrm{out}}_e$ and predicates $\mathrm{pre}_e,\mathrm{post}_e$ encode transition admissibility.
...
\end{equation}
where $y_t$ is normalized output and $\Phi_t$ is the validator outcome tuple. Admissibility and commit rules are
\begin{equation}
...
For horizon $H$, we define
\begin{equation}
J_1(\Pi,\Phi^{\mathrm{hard}})=\sum_{t=0}^{H-1}\mathbf{1}\!\left[\mathrm{commit}_t \land \neg \mathrm{Safe}(s_t)\right],
...
We define
\begin{equation}
J_2(\Pi)=\mathbb{E}\!\left[\sum_{t=0}^{H-1}c(a_t,s_t)\right]+\lambda\,\mathbb{E}[N_{\mathrm{viol}}],
\label{eq:objective_j2}
\end{equation}
with feasibility constraints
\begin{equation}
\mathcal{F}_2=\left\{\Pi:\ N_{\mathrm{rb}}(\omega)\le B\ \forall\omega,\ V(s_{t+1})\le V(s_t)-\delta\right\}.
\label{eq:feasible_f2}
\end{equation}
Here $N_{\mathrm{rb}}$ counts reroute/backtrack actions and $\delta>0$ enforces strict potential descent. \Eqref{eq:objective_j2} and \eqref{eq:feasible_f2} are used to derive non-oscillation and bounded correction length in \secref{sec:formal}.
...
\end{equation}
The feasible set encodes C1--C4, including append-only hash-link invariance. We emphasize provenance in this objective because protocol-level exploit analyses show trust-only gating is insufficient \citep{ferrag2026,ismail2025,gao2025}.
...
Assume C1 and a common attack distribution over covered classes. Let $A=\{\mathrm{trust}(y)\ge\theta\}$ and $B=\{\psi_{\kappa}(y)=1\}$. Then acceptance under composite guard \eqref{eq:guard} is $A\cap B \subseteq A$, implying
\begin{equation}
...
\end{theorem}
\begin{proof}
Set inclusion $A\cap B\subseteq A$ is immediate from conjunction. Under a fixed covered-class distribution, event probability is monotone under set inclusion, so $\mathbb{P}(A\cap B)\le\mathbb{P}(A)$. Interpreting these probabilities as false-accept rates for covered attacks yields \eqref{eq:far_contraction}.\qedhere
\end{proof}
...
\end{equation}
where $H$ is collision-resistant for the operational threat model, committed traces preserve append-only hash-link integrity across continue, reroute, and backtrack operations.
\end{theorem}
\begin{proof}
...
\caption{Unsafe-commit outcomes across validator modes and assumption toggles for the committed-prefix safety claim. The horizontal axis indexes validator composition regimes and the vertical axis reports unsafe-commit rate with seed-level variability. The key interpretation is regime-specific: compositional semantic-and-typed validation under active fidelity assumptions reaches zero unsafe commits, while schema-only and weakened-assumption settings produce deterministic failures that match the counterexample boundary.}
\label{fig:unsafe_commit}
\end{figure}

\Figref{fig:unsafe_commit} directly tests the condition in \eqref{eq:objective_j1}. In the compositional regime that satisfies A1--A4, unsafe commits are eliminated. When validator fidelity is disabled or non-compositional gates are used, unsafe commits reappear, consistent with Corollary~\ref{cor:ce1}. This pattern supports the theorem as assumption-scoped rather than universal.
...
\begin{table}[t]
\caption{Recovery summary across policy baselines. Metrics report oscillation, bound violations, and regret relative to rollback comparator behavior. The table corroborates that bounded policies remain stable under theorem conditions, while unbounded branching acts as a falsifier regime that quantifies boundary failure.}
...
\begin{table}[t]
\caption{Attack-class coverage and integrity outcomes for security/provenance evaluation. Rows separate guard composition and attack-class scope, while columns report false acceptance, false rejection, and trace-integrity pass rates. The table makes theorem boundaries explicit by showing performance gains on covered classes and residual risk on uncovered classes.}
