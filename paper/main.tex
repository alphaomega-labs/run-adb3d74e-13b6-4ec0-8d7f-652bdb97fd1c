...
We use this structure to keep citation use disciplined: prior work informs assumptions and threat models, while report-defined objectives and proofs provide the claim core. This avoids conflating literature consensus with theorem guarantees and keeps responsibility for new claims explicit.
...
Formal-method and symbolic-control sources suggest reading this space through three layers. The first layer asks whether local transition conditions are explicit and checkable. The second asks whether those local conditions compose into global statements over committed traces. The third asks whether provenance remains invariant under corrective control so that downstream scientific claims stay auditable \citep{song2026b,sawant2026,dinu2024,glas2026}. Existing papers frequently cover one or two layers well, but fewer provide closure across all three. This layered view motivates our report design: we separate local safety, bounded recovery, and security--provenance arguments, then reconnect them through shared objectives and feasibility constraints.
...
This report addresses that gap through a hybrid contribution boundary: a formal core with explicit assumptions and proofs, supported by executed theorem-audit experiments and symbolic checks that expose both supported regimes and failure boundaries.
...
\end{equation}
Equations \eqref{eq:update_operator}--\eqref{eq:append_rule} are report-defined algebraic objects that operationalize sourced validator-gated conventions \citep{sawant2026,chang2025,song2026b}.
...
\label{sec:notation}
Table~\ref{tab:notation} summarizes recurrent symbols after they have been introduced in context. The placement is deliberate. In dense formal reports, readers often need a compact lookup while evaluating proof steps, but placing notation before definitions can obscure role-specific semantics. We therefore define objects first and then consolidate them. This keeps the narrative forward-moving while preserving quick reference support for later theorem and evidence sections.
The table also encodes provenance at the symbol level. Graph, contract, and validator primitives follow conventions from the cited orchestration and formal-method literature, while objective symbols and some audit quantities are report-defined. Distinguishing these categories supports reproducibility: future work can adopt inherited conventions without reproducing every modeling decision, and can separately test whether report-defined objectives remain appropriate in new domains.
\begin{table}[t]
\caption{Core notation used in the formal development. The table is placed after definitions so symbols are disambiguated in context. Symbols marked ``defined here'' are report-specific formal objects, while others are inherited conventions adapted from cited orchestration and formal-method sources.}
\label{tab:notation}
\centering
\small
...
This result sharpens the report's scope claim. The evidence supports an assumption-qualified statement: zero unsafe commits are obtained when A1--A4 hold and validator composition is enforced; boundary regimes produce the predictable failures described by the corollary.
...
The combined formal and empirical picture is deliberately non-triumphal. The report does not claim unconditional guarantees; it shows that guarantees are obtainable when assumptions are explicit, auditable, and tested against targeted falsifiers. This distinction matters for long-horizon autonomous research systems, where unchecked generalization from positive runs can mask regime fragility.
...
The experiment driver records a manifest per run that includes assumption toggles, scenario family, seed, and output hashes for generated summaries. This manifest is used as the join key across runtime traces, theorem-audit summaries, and report tables so that each reported statistic can be traced to concrete executable evidence. We avoid ad hoc postprocessing by requiring metric aggregation scripts to consume only manifest-indexed outputs.
...
This report adopts validator-gated workflow and protocol-threat conventions from prior work \citep{sawant2026,chang2025,ferrag2026,glas2026,dinu2024}, and defines in this work the objective family $J_1,J_2,J_3$ plus feasible sets used for theorem statements. The equation lineage is therefore hybrid: sourced assumptions with report-defined optimization formalism.
Because this hybrid lineage can be misunderstood, we explicitly separate inherited conventions from report-defined equations in the traceability map and theorem-audit reports. This documentation choice makes it possible to audit originality claims and to determine which dependencies must be revalidated when upstream comparator assumptions change.
